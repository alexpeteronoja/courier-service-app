...
const mockShipments = {
  SS123456789: {
    trackingCode: "SS123456789",
    status: "out_for_delivery",
    sender: {
      name: "Amazon Warehouse NYC",
    ...
    <div className="pt-24 pb-10 ml-0 lg:ml-71 px-4 md:px-8.5 bg-background text-textcol min-h-dvh">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              <Info
                label="Ship Date"
                value={shipment.details.shipDate}
                icon={Calendar}
              />
              <Info
                label="Estimated Delivery"
                value={shipment.details.estimatedDelivery}
                icon={Calendar}
              />
              <Info
                label="Service"
                value={shipment.details.service}
                icon={Package}
              />
              <Info
                label="Current Location"
                value={shipment.details.currentLocation}
                icon={MapPin}
              />
              <Info label="Weight" value={shipment.details.weight} />
              <Info label="Dimensions" value={shipment.details.dimensions} />
            </div>
          </div>
        ...
        <div className="space-y-6">
          {/* Sender */}
          <PersonCard title="Sender" icon={User} person={shipment.sender} />

          {/* Receiver */}
          <PersonCard
            title="Receiver"
            icon={MapPin}
            person={shipment.receiver}
            showContact
          />
        </div>
      </div>
